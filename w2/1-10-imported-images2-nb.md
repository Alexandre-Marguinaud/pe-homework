...
def patchwork(c, side=10, background=[169, 169, 169]):
    """
    ...
    """
    # your code here
    C = np.array(c+[background])
    a, b = rectangle_size(len(c))
    T = np.indices((a*side, b*side))
    Refs = np.minimum(T[0]//side*b+T[1]//side, len(c)).astype(int)
    print(Refs)
    resu = C[Refs]
    return resu
```
...
# votre code
couleurs = {}
f = open('rgb-codes.txt','r')
ligne = f.readline()
L = ligne.split()
while len(L) == 4:
    couleurs[L[0]] = [int(L[1]), int(L[2]), int(L[3])]
    ligne = f.readline()
    L = ligne.split()
...
def patchwork2(color_names, side, background_color):
    '''
    ...
    '''
    # your goes goes here
    C = np.array([couleurs[color_names[i]] for i in range(len(color_names))]+[couleurs[background_color]])
    a, b = rectangle_size(len(color_names))
    T = np.indices((a*side, b*side))
    Refs = np.minimum(T[0]//side*b+T[1]//side, len(color_names)).astype(int)
    resu = C[Refs].astype(np.uint8)
    return resu
```

```{code-cell} ipython3
# ou encore
plt.imshow(patchwork2(color_names, 20, 'DarkGray'));
```
...
color_names = []
n = 140
liste = list(couleurs.keys())
color_names = rd.sample(liste, n)
```
...
def selection(mot):
    ...
    for i in range(n):
        compteur = 0
        for j in range(len(color_names[i])):
            if color_names[i][j] == mot[compteur]:
                compteur += 1
                if compteur == len(mot)-1:
                    colors_searched.append(color_names[i])
                    break
            else: compteur = 0
    return colors_searched

plt.imshow(patchwork2(selection('White'), 20, 'DarkGray'))
```

```{code-cell} ipython3
plt.imshow(patchwork2(selection('Yellow'), 20, 'DarkGray'))
```
...
```{code-cell} ipython3
#plt.imshow(patchwork2(liste, 20, 'DarkGray'))
plt.imsave('patchwork.png', patchwork2(liste, 20, 'DarkGray'))
```
...
```{code-cell} ipython3
plt.imshow(plt.imread('patchwork.png'))
```
...
```{code-cell} ipython3
def sepia(RGB): #version brutale
    tab = plt.imread(RGB)
    R, G, B = tab[:,:,0], tab[:,:,1], tab[:,:,2]
    resu = tab.astype(int)
    resu[:,:,0] = 0.393 * R + 0.769 * G + 0.189 * B
    resu[:,:,1] = 0.349 * R + 0.686 * G + 0.168 * B
    resu[:,:,2] = 0.272 * R + 0.534 * G + 0.131 * B
    return resu

def sepia2(RGB): #version plus élégante
    tab = plt.imread(RGB)
    coeffs = np.array([[0.393, 0.769, 0.189], [0.349, 0.686, 0.168], [0.272, 0.534, 0.131]])
    resu = tab@coeffs.T
    resu = np.minimum(tab@coeffs.T, 255) #coeffs < 255
    return resu.astype(int)
```
...
plt.imshow(sepia2("les-mines.jpg"))
#plt.show()
```
